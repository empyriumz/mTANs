...
if __name__ == "__main__":
    ...
    for itr in range(1, args.niters + 1):
        ...
        val_loss, val_acc, val_auc = utils.evaluate_classifier(
            rec, val_loader, args=args, dim=dim
        )
        best_val_loss = min(best_val_loss, val_loss)
        test_loss, test_acc, test_auc = utils.evaluate_classifier(
            rec, test_loader, args=args, dim=dim
        )
        print(
            "Iter: {}, loss: {:.4f}, acc: {:.4f}, val_loss: {:.4f}, val_acc: {:.4f}, test_acc: {:.4f}, test_auc: {:.4f}".format(
                itr,
                train_loss / train_n,
                train_acc / train_n,
                val_loss,
